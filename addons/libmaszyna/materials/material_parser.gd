...
func parse(file_path:String) -> MaszynaMaterial:
    var file = FileAccess.open(file_path, FileAccess.READ) as FileAccess
    ...
    if file:
        ...
        if _t1 is Array:
            push_warning("Material: " + file_path +": more than 1 texture parameters aren't supported!")
            _t1 = _t1[0]
        if _t2 is Array:
            push_warning("Material: " + file_path +": more than 1 texture parameters aren't supported!")
            _t2 = _t2[0]
