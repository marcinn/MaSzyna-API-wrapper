...
namespace Maszyna {
    ...
    std::string ExchangeCharInString(std::string const &Source, char const From, char const To) {
        ...
        std::for_each(std::begin(Source), std::end(Source), [&](char const idx) {
            if (idx != From) {
                replacement += idx;
            } else {
                replacement += To;
    ...
    std::vector<std::string> &Split(const std::string &s, char delim,
                                    std::vector<std::string> &elems) { // dzieli tekst na wektor tekstow
    ...
    std::vector<std::string> Split(const std::string &s, char delim) { // dzieli tekst na wektor tekstow
        std::vector<std::string> elems;
        Split(s, delim, elems);
        return elems;
    }

    std::vector<std::string> Split(const std::string &s) { // dzieli tekst na wektor tekstow po białych znakach
        std::vector<std::string> elems;
        std::stringstream ss(s);
        std::string item;
            while (ss >> item) {
    ...
    std::string to_minutes_str(float const Minutes, bool const Leadingzero, int const Width) {
        ...
        auto minutes = (std::string(width - 1, ' ') +
                        (Leadingzero ? to_string(100 + minutesintegral).substr(1, 2) : to_string(minutesintegral, 0)));
        return (minutes.substr(minutes.size() - width, width) +
    ...
    std::string ToLower(std::string const &text) {
        ...
        auto lowercase{text};
        std::transform(std::begin(text), std::end(text), std::begin(lowercase),
    ...
    std::string ToUpper(std::string const &text) {
        ...
        auto uppercase{text};
        std::transform(std::begin(text), std::end(text), std::begin(uppercase),
    ...
    void win1250_to_ascii(std::string &Input) {
        ...
        std::unordered_map<char, char> const charmap{{165, 'A'}, {198, 'C'}, {202, 'E'}, {163, 'L'}, {209, 'N'},
                                                     {211, 'O'}, {140, 'S'}, {143, 'Z'}, {175, 'Z'}, {185, 'a'},
                                                     {230, 'c'}, {234, 'e'}, {179, 'l'}, {241, 'n'}, {243, 'o'},
                                                     {156, 's'}, {159, 'z'}, {191, 'z'}};
    ...
    std::string win1250_to_utf8(const std::string &Input) {
        std::unordered_map<char, std::string> const charmap{
                {165, u8"Ą"}, {198, u8"Ć"}, {202, u8"Ę"}, {163, u8"Ł"}, {209, u8"Ń"}, {211, u8"Ó"},
                {140, u8"Ś"}, {143, u8"Ź"}, {175, u8"Ż"}, {185, u8"ą"}, {230, u8"ć"}, {234, u8"ę"},
                {179, u8"ł"}, {241, u8"ń"}, {243, u8"ó"}, {156, u8"ś"}, {159, u8"ź"}, {191, u8"ż"}};
        std::string output;
    ...
    bool erase_extension(std::string &Filename) {
        ...
        auto const extensionpos{Filename.rfind('.')};
            ...
            if (extensionpos != Filename.rfind("..") + 1) {
                // we can get extension for .mat or, in legacy files, some image format. just trim it and set it to
    ...
    int count_trailing_zeros(unsigned long val) {
        ...
        for (unsigned long shift = 1; !(val & shift); shift <<= 1)
            r++;
