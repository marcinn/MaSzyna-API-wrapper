...
#define p_slippdmg (1e-03)
    ...
    enum class range_t { local, unit, consist };
    ...
    /*tory*/
    struct TTrackShape {          /*ksztalt odcinka*/
    ...
    struct TTrackParam { /*parametry odcinka - szerokosc, tarcie statyczne, kategoria,  obciazalnosc w t/os,
                            uszkodzenia*/
            double Width = 0.0;
            double friction = 0.0;
            int CategoryFlag = 0;
    ...
    struct TMPTRelay { /*lista przekaznikow bocznikowania*/
            double Iup = 0.0;
            double Idown = 0.0;
    };
    ...
    struct TTransmision { // liczba zebow przekladni
            int NToothM = 0;
            int NToothW = 0;
            double Ratio = 1.0;
    ...
    class TMoverParameters { // Ra: wrapper na kod pascalowy, przejmujący jego funkcje  Q: 20160824 - juz nie wrapper a
                             // klasa bazowa :)
        private:
            // types
            /* TODO: implement
                ...
                // members
                    ...
                    // boolean pairs, exchanged data is swapped when connected to a matching end (front to front or back
               to back)
            ...
            */
            ...
            struct basic_door {
                    ...
                    // internal data
                    float auto_timer{
                            -1.f}; // delay between activation of open state and closing state for automatic doors
            ...
            struct heat_data {
                    ...
                    double fan_speed{
                            0.075}; // cooling fan rpm; either fraction of engine rpm, or absolute value if negative
        ...
        public:
            ...
            int EmergencyBrakeWarningSignal{
                    0};                      // combined with basic WarningSignal when manual emergency brake is active
            ...
            bool UniCtrlIntegratedLocalBrakeCtrl =
            ...
            std::pair<std::string, std::array<int, 2>> PantsPreset{
                    "0132", {0, 0}}; // pantograph preset switches; .first holds possible setups as chars, .second holds
                                     // currently selected preset in each cab
            ...
            double u = 0.0;          // wspolczynnik tarcia yB
                                     // wywalic!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            int CompressorPower = 1; // 0: main circuit, 1: z przetwornicy, reczne, 2: w przetwornicy, stale, 3: diesel
                                     // engine, 4: converter of unit in front, 5: converter of unit behind
            ...
            /*                dizel_auto_min, dizel_auto_max: real; {predkosc obrotowa przelaczania automatycznej
             * skrzyni biegow*/
            ...
            double dizel_nmin_hdrive_factor =
                    0.0; /*wspolczynnik wzrostu obrotow minimalnych hydro zaleznosci od zadanego procentu*/
            ...
            double dizel_NominalFuelConsumptionRate =
                    250.0; /*jednostkowe zużycie paliwa przy mocy nominalnej, wczytywane z fiz, g/kWh*/
            double dizel_FuelConsumption =
                    0.0; /*współczynnik zużycia paliwa przeliczony do jednostek maszynowych, l/obrót*/
            ...
            double hydro_TC_TorqueInOut =
                    0.0; /*stala momentu proporcjonalnego do roznica obrotow wejsciowych i wyjsciowych*/
            ...
            int hydro_R_Placement = 0; /*umiejscowienie retardera: 0 - za skrzynia biegow, 1 - miedzy przetwornikiem a
                                          biegami, 2 - przed skrzynia biegow */
            ...
            double DCEMUED_EP_max_Vel{
                    0.0}; // maksymalna prędkość, przy której działa EP przy włączonym ED w jednostce (dla tocznych)
            double DCEMUED_EP_min_Im{
                    0.0}; // minimalny prąd, przy którym EP nie działa przy włączonym ED w członie (dla silnikowych)
            ...
            bool DoorClosureWarningAuto =
                    false; // departure signal plays automatically while door closing button is held down
            ...
            float DoorCloseDelay{
                    0.f}; // delay (in seconds) before the door begin closing, once conditions to close are met
            ...
            /*objetosc spr. powietrza w ukl. pantografu*/ // aby podniesione pantografy opadły w krótkim czasie przy
            ...
            bool CompressorAllowLocal{true}; // local device state override (most units don't have this fitted so it's
                                             // set to true not to intefere)
            bool CompressorGovernorLock{false}; // indicates whether compressor pressure switch was activated due to
                                                // reaching cut-out pressure
            bool CompressorTankValve{
                    false}; // indicates excessive pressure is vented from compressor tank directly and instantly
            ...
            float ConverterStartDelay{
                    0.0f}; // delay (in seconds) before the converter is started, once its activation conditions are met
            ...
            bool ConverterAllowLocal{true}; // local device state override (most units don't have this fitted so it's
                                            // set to true not to intefere)
            bool ConverterFlag = false;     /*!  czy wlaczona przetwornica NBMX*/
            start_t ConverterOverloadRelayStart{
                    start_t::manual}; // whether overload relay reset responds to dedicated button
            ...
            door_data Doors;
            float DoorsOpenWithPermitAfter{-1.f}; // remote open if permit button is held for specified time. NOTE:
                                                  // separate from door data as its cab control thing
            ...
            bool UniversalBrakeButtonActive[3] = {false, false, false}; /* brake button pressed */
                                                                        /*
                                                                            int BrakeStatus = b_off; //0 - odham, 1 - ham., 2 - uszk., 4 - odluzniacz, 8 - antyposlizg, 16 - uzyte
            ...
            double MainsInitTime{0.0}; // config, initialization time (in seconds) of the main circuit after it receives
                                       // power, before it can be closed
            double MainsInitTimeCountdown{
                    0.0}; // current state of main circuit initialization, remaining time (in seconds) until it's ready
            start_t MainsStart{start_t::manual};
            bool LineBreakerClosesOnlyAtNoPowerPos{false};
            bool ControlPressureSwitch{false}; // activates if the main pipe and/or brake cylinder pressure aren't
                                               // within operational levels
            ...
            int DirActive = 0; // czy lok. jest wlaczona i w ktorym kierunku: względem wybranej kabiny: -1 - do tylu, +1
                               // - do przodu, 0 - wylaczona
            int DirAbsolute = 0; // zadany kierunek jazdy względem sprzęgów (1=w strone 0,-1=w stronę 1)
            int MainCtrlMaxDirChangePos{
                    0}; // can't change reverser state with master controller set above this position
            int CabActive =
                    0; // numer kabiny, z której jest sterowanie: 1 lub -1; w przeciwnym razie brak sterowania - rozrzad
            int CabOccupied =
                    0; // numer kabiny, w ktorej jest obsada (zwykle jedna na skład) // TODO: move to TController
            ...
            bool PantPressSwitchActive{false}; // state of the pantograph pressure switch. gets primed at defined
                                               // pressure level in pantograph air system
            bool PantPressLockActive{false};   // pwr system state flag. fires when pressure switch activates by
                                               // pantograph pressure dropping below defined level
            ...
            std::array<std::pair<double, double>, 4> PowerCircuits; // 24v, 110v, 3x400v and 3000v power circuits,
                                                                    // voltage from local sources and current draw pairs

            /*-zmienne dla lokomotywy spalinowej z przekladnia mechaniczna*/
            double dizel_fill = 0.0; /*napelnienie*/
            double dizel_engagestate =
                    0.0; /*sprzeglo skrzyni biegow: 0 - luz, 1 - wlaczone, 0.5 - wlaczone 50% (z poslizgiem)*/
            ...
            bool bPantKurek3 = true; // kurek trójdrogowy (pantografu): true=połączenie z ZG, false=połączenie z małą
                                     // sprężarką // domyślnie zbiornik pantografu połączony jest ze zbiornikiem głównym
        ...
        public:
            ...
            bool
            Attach(int ConnectNo, int ConnectToNr, TMoverParameters *ConnectTo, int CouplingType, bool Enforce = false,
                   bool Audible = true);
            ...
            double ComputeMovement(
                    double dt, double dt1, const TTrackShape &Shape, TTrackParam &Track,
                    TTractionParam &ElectricTraction, TLocation const &NewLoc,
                    TRotation const &NewRot); // oblicza przesuniecie pojazdu
            double FastComputeMovement(
                    double dt, const TTrackShape &Shape, TTrackParam &Track, TLocation const &NewLoc,
                    TRotation const &NewRot); // oblicza przesuniecie pojazdu - wersja zoptymalizowana
            ...
            double EngineRPMRatio()
                    const; // returns current engine revolutions as percentage of max engine revolutions, in range 0-1
            double EngineIdleRPM() const;
            double EngineMaxRPM() const;

            /*! przesylanie komend sterujacych*/
            bool SendCtrlToNext(
                    std::string const CtrlCommand, double const ctrlvalue, double const dir,
                    int const Couplertype = coupling::control);
            bool SetInternalCommand(
                    std::string NewCommand, double NewValue1, double NewValue2,
                    int const Couplertype = coupling::control);
            double GetExternalCommand(std::string &Command);
            bool
            RunCommand(std::string Command, double CValue1, double CValue2, int const Couplertype = coupling::control);
            ...
            bool IsMainCtrlActualNoPowerPos() const; // whether the master controller is actually set to position which
                                                     // won't generate any extra power
            bool IsMainCtrlNoPowerPos()
                    const; // whether the master controller is set to position which won't generate any extra power
            bool IsMainCtrlMaxPowerPos() const;
            int MainCtrlNoPowerPos()
                    const; // highest setting of master controller which won't cause engine to generate extra power
            int MainCtrlActualPowerPos() const; // current actual setting of master controller, relative to the highest
                                                // setting not generating extra power
            int MainCtrlPowerPos() const; // current setting of master controller, relative to the highest setting not
            ...
            bool SandboxManual(
                    bool const State, range_t const Notify = range_t::consist); /*wlacza/wylacza reczne sypanie piasku*/
            bool SandboxAuto(
                    bool const State,
                    range_t const Notify = range_t::consist); /*wlacza/wylacza automatyczne sypanie piasku*/
            ...
            bool WaterPumpBreakerSwitch(
                    bool State,
                    range_t const Notify = range_t::consist); // water pump breaker state toggle
            ...
            bool WaterHeaterBreakerSwitch(
                    bool State,
                    range_t const Notify = range_t::consist); // water heater breaker state toggle
            bool WaterHeaterSwitch(bool State, range_t const Notify = range_t::consist); // water heater state toggle
            bool WaterCircuitsLinkSwitch(
                    bool State,
                    range_t const Notify = range_t::consist); // water circuits link state toggle
            ...
            bool MotorBlowersSwitch(
                    bool State, end const Side,
                    range_t const Notify = range_t::consist); // traction motor fan state toggle
            bool MotorBlowersSwitchOff(
                    bool State, end const Side,
                    range_t const Notify = range_t::consist); // traction motor fan state toggle
            bool CompartmentLightsSwitch(
                    bool State,
                    range_t const Notify = range_t::consist); // compartment lights state toggle
            bool CompartmentLightsSwitchOff(
                    bool State,
                    range_t const Notify = range_t::consist); // compartment lights state toggle
            ...
            bool MaxCurrentSwitch(
                    bool State,
                    range_t const Notify = range_t::consist); // przelacznik pradu wysokiego rozruchu
            ...
            bool
            OperatePantographValve(end const End, operation_t const State, range_t const Notify = range_t::consist);
            ...
            bool signal_departure(
                    bool const State,
                    range_t const Notify = range_t::consist); // toggles departure warning
        ...
        private:
            /*
                ...
                void LoadFIZ_PowerParamsDecode(TPowerParameters &Powerparameters, std::string const Prefix,
                ...
                bool readBPT(std::string const &line); // Q 20160721
