...
namespace Maszyna {
	...
	double TFricMat::GetFC(double N, double Vel) { return 1; }
	...
	double TP10Bg::GetFC(const double N, const double Vel) {
		...
		return exp(-0.022 * N) * (0.19 - 0.095 * exp(-Vel * 1.0 / 25.7)) + 0.384 * exp(-Vel * 1.0 / 25.7) - 0.028;
	}
	...
	double TP10Bgu::GetFC(const double N, const double Vel) {
		...
		return exp(-0.017 * N) * (0.18 - 0.09 * exp(-Vel * 1.0 / 25.7)) + 0.381 * exp(-Vel * 1.0 / 25.7) -
			   0.022; // 0.05*exp(-0.2*N);
	}

	double TP10yBg::GetFC(const double N, const double Vel) {
		const double A = 2.135 * exp(-0.03726 * N) - 0.5;
		const double C = 0.353 - A * 0.029;
		const double u0 = 0.41 - C;
		const double V0 = 25.7 + 20 * A;
		return u0 + C * exp(-Vel * 1.0 / V0);
	}

	double TP10yBgu::GetFC(const double N, const double Vel) {
		const double A = 1.68 * exp(-0.02735 * N) - 0.5;
		const double C = 0.353 - A * 0.044;
		const double u0 = 0.41 - C;
		const double V0 = 25.7 + 21 * A;
		return u0 + C * exp(-Vel * 1.0 / V0);
	}

	double TP10::GetFC(const double N, const double Vel) {
		return 0.60 * ((1.6 * N + 100) * 1.0 / (8.0 * N + 100)) * ((Vel + 100) * 1.0 / (5 * Vel + 100));
	...
	double TCosid::GetFC(double N, double Vel) { return 0.27; }

	double TDisk1::GetFC(const double N, double Vel) { return 0.2375 + 0.000885 * N - 0.000345 * N * N; }

	double TDisk2::GetFC(double N, double Vel) { return 0.27; }

	double TFR510::GetFC(double N, double Vel) { return 0.15; }
