...
namespace Maszyna {
	double d2A(double const d) { return d * d * 0.7854 / 1000.0; }

	// ------ RURA ------

	double TRura::P(void) { return Next->P(); }
	...
	void TPrzeciwposlizg::SetSlippery(const bool flag) { Slippery = flag; }
	...
	void TRapid::SetRapidStatus(const bool rs) { RapidStatus = rs; }
	...
	void TPrzekCiagly::SetMult(const double m) { Mult = m; }
	...
	void TPrzek_PZZ::SetLBP(const double P) { LBP = P; }
	...
	void TPrzekED::SetP(const double P) { MaxP = P; }
	...
	double TNESt3::GetPF(double const PP, double const dt, double const Vel) // przeplyw miedzy komora wstepna i PG
	{
		...
		double const nastG = BrakeDelayFlag & bdelay_G;
		...
		if ((BrakeStatus & b_hld) == b_off)
			dV = PF(0.0, BCP, Nozzles[dTO] * nastG + (1.0 - nastG) * Nozzles[dOO]) * dt *
		...
		if ((BrakeStatus & b_on) == b_on && Przekladniki[1]->P() * HBG300 < MaxBP)
			dV = PF(BVP, BCP,
					Nozzles[dTN] * (nastG + 2.0 * (BCP < Podskok ? 1.0 : 0.0)) + Nozzles[dON] * (1.0 - nastG)) *
		...
		for (int i = 1; i < 4; ++i) {
			...
			if (typeid(*Przekladniki[i]) == typeid(TRapid)) {
				RapidStatus = (BrakeDelayFlag & bdelay_R) == bdelay_R &&
							  (std::abs(Vel) > 70.0 || (std::abs(Vel) > 50.0 && RapidStatus) || RapidStaly);
				Przekladniki[i]->SetRapidStatus(RapidStatus);
			} else if (typeid(*Przekladniki[i]) == typeid(TPrzeciwposlizg))
				Przekladniki[i]->SetSlippery((BrakeStatus & b_asb) == b_asb);
	...
	/**
	 *
	 * @param BCP Główny przyrząd rozrządczy
	 * @param dV1
	 */
	void TNESt3::CheckState(double const BCP, double &dV1) {
		double const VVP{ValveRes->P()};
		double const CVP{CntrlRes->P()};
		double const MPP{Miedzypoj->P()};
	 ...
	 * @param dt
	 */
	void TNESt3::CheckReleaser(double const dt) {
		double const VVP{ValveRes->P()};
		double const CVP{CntrlRes->P()};
	...
	/**
	 * Napełniacz sterującego
	 * @param BP
	 * @return
	 */
	double TNESt3::CVs(double const BP) const {
		double const CVP{CntrlRes->P()};
		double const MPP{Miedzypoj->P()};
	...
	/**
	 * Napełniacz pomocniczego
	 * @param BCP
	 * @return
	 */
	double TNESt3::BVs(double const BCP) const {
		double const CVP{CntrlRes->P()};
		double const MPP{Miedzypoj->P()};
	...
	void TNESt3::PLC(double const mass) {
		LoadC = 1.0 +
				(mass < LoadM ? (TareBP + (MaxBP - TareBP) * (mass - TareM) / (LoadM - TareM)) / MaxBP - 1.0 : 0.0);
	}
	...
	void TNESt3::SetLBP(double const P) { LBP = P; }

	bool contains(const std::string &string, const char *str) { return strstr(string.c_str(), str); };
	...
	void TNESt3::SetSize(int const size, std::string const &params) // ustawianie dysz (rozmiaru ZR)
	{
		...
		if (contains(params, "3d") || contains(params, "4d")) {
			autom = false;
		} else
			autom = true;
