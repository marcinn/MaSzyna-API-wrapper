...
namespace Maszyna {
	...
	double Max0R(const double x1, const double x2) {
		if (x1 > x2)
			return x1;
		return x2;
	}

	double Min0R(const double x1, const double x2) {
		if (x1 < x2)
			return x1;
		return x2;
	}
	...
	bool is_EQ(const double pos, const double i_pos) { return pos <= i_pos + 0.5 && pos > i_pos - 0.5; }

	//---ZBIORNIKI---

	double TReservoir::pa() { return 0.1 * Vol / Cap; }

	double TReservoir::P() { return Vol / Cap; }

	void TReservoir::Flow(const double dv) { dVol = dVol + dv; }
	...
	void TReservoir::CreateCap(const double Capacity) { Cap = Capacity; }
	...
	double TBrake::GetBCP() const { return BrakeCyl->P(); }

	// ciśnienie sterujące hamowaniem elektro-dynamicznym
	double TBrake::GetEDBCP() { return 0; }

	// ciśnienie zbiornika pomocniczego
	double TBrake::GetBRP() const { return BrakeRes->P(); }

	// ciśnienie komory wstępnej
	double TBrake::GetVRP() const { return ValveRes->P(); }

	// ciśnienie zbiornika sterującego
	double TBrake::GetCRP() { return GetBRP(); }
	...
	double TBrake::GetHPFlow(double const HP, double const dt) { return 0; }

	double TBrake::GetBCF() const { return BCA * 100 * BrakeCyl->P(); }
	...
	void TBrake::Releaser(int const state) { BrakeStatus = BrakeStatus & ~b_rls | state * b_rls; }

	bool TBrake::Releaser() const { return (BrakeStatus & b_rls) == b_rls; }
	...
	int TBrake::GetStatus() const { return BrakeStatus; }
	...
	void TBrake::SetASBP(double const Press) { ASBP = Press; }
	...
	double TWest::GetHPFlow(double const HP, double const dt) { return dVP; }
	...
	double TESt::GetCRP() { return CntrlRes->P(); }
	...
	Type_ clamp(Type_ const Value, Type_ const Min, Type_ const Max) {
		Type_ value = Value;
		if (value < Min) {
			value = Min;
		}
	...
	void TEStEP1::SetEPS(double const nEPS) { EPS = nEPS; }
	...
	double TLSt::GetPF(double const PP, double const dt, double const Vel) {
		...
		if (VVP > BCP) {
			...
			dV = PF(VVP, BCP,
					0.001472 * (1.36 - (true == ((CVP - BCP) * BVM > 1.0 && BrakeDelayFlag == bdelay_G) ? 1.0 : 0.0)),
					0.1) *
				 dt;
		} else {
			dV = 0;
	...
	void TLSt::SetLBP(double const P) { LBP = P; }
	...
	void TLSt::SetED(double const EDstate) { EDFlag = EDstate; }

	void TLSt::SetRM(double const RMR) { RM = 1 - RMR; }
	...
	double TEStED::GetEDBCP() { return ImplsRes->P() * LoadC; }
	...
	double TCV1::GetCRP() { return CntrlRes->P(); }
	...
	void TCV1L_TR::SetLBP(double const P) { LBP = P; }
	...
	double TKE::GetCRP() { return CntrlRes->P(); }
	...
	void TKE::SetRM(double const RMR) { RM = 1.0 - RMR; }

	void TKE::SetLBP(double const P) { LBP = P; }
	...
	double TDriverHandle::GetPF(double const i_bcp, double PP, double HP, double dt, double ep) { return 0; }
	...
	double TDriverHandle::GetCP() { return 0; }

	double TDriverHandle::GetEP() { return 0; }

	double TDriverHandle::GetRP() { return 0; }

	double TDriverHandle::GetSound(int i) { return 0; }

	double TDriverHandle::GetPos(int i) { return 0; }

	double TDriverHandle::GetEP(double pos) { return 0; }

	void TDriverHandle::OvrldButton(const bool Active) { ManualOvrldActive = Active; }

	void TDriverHandle::SetUniversalFlag(const int flag) { UniversalFlag = flag; }
	...
	void TFV4aM::SetReductor(const double nAdj) { RedAdj = nAdj; }
	...
	double TFV4aM::GetPos(const int i) { return pos_table[i]; }

	double TFV4aM::GetCP() { return TP; }

	double TFV4aM::GetRP() { return 5.0 + TP * 0.08 + RedAdj; }
	...
	bool TFV4aM::EQ(const double pos, const double i_pos) { return pos <= i_pos + 0.5 && pos > i_pos - 0.5; }
	...
	void TMHZ_EN57::Init(const double Press) { CP = Press; }

	void TMHZ_EN57::SetReductor(const double nAdj) { RedAdj = nAdj; }
	...
	double TMHZ_EN57::GetPos(const int i) { return pos_table[i]; }

	double TMHZ_EN57::GetCP() { return RP; }

	double TMHZ_EN57::GetRP() { return 5.0 + RedAdj; }
	...
	bool TMHZ_EN57::EQ(const double pos, const double i_pos) { return pos <= i_pos + 0.5 && pos > i_pos - 0.5; }
	...
	void TMHZ_K5P::SetReductor(const double nAdj) { RedAdj = nAdj; }
	...
	double TMHZ_K5P::GetPos(const int i) { return pos_table[i]; }

	double TMHZ_K5P::GetCP() { return CP; }

	double TMHZ_K5P::GetRP() { return 5.0 + TP + RedAdj; }
	...
	bool TMHZ_K5P::EQ(const double pos, const double i_pos) { return pos <= i_pos + 0.5 && pos > i_pos - 0.5; }
	...
	void TMHZ_6P::SetReductor(const double nAdj) { RedAdj = nAdj; }
	...
	double TMHZ_6P::GetPos(const int i) { return pos_table[i]; }

	double TMHZ_6P::GetCP() { return CP; }

	double TMHZ_6P::GetRP() { return 5.0 + TP + RedAdj; }
	...
	bool TMHZ_6P::EQ(const double pos, const double i_pos) { return pos <= i_pos + 0.5 && pos > i_pos - 0.5; }
	...
	double TM394::GetPF(const double i_bcp, const double PP, const double HP, const double dt, double ep) {
		...
		if (BCP != 2)
			if (CP < LimPP)
				CP = CP + 4 * Min0R(abs(LimPP - CP), 0.05) * PR(CP, LimPP) * dt; // zbiornik sterujący
	...
	void TM394::SetReductor(const double nAdj) { RedAdj = nAdj; }

	double TM394::GetCP() { return CP; }

	double TM394::GetRP() { return std::max(5.0, CP) + RedAdj; }

	double TM394::GetPos(const int i) { return pos_table[i]; }
	...
	void TH14K1::SetReductor(const double nAdj) { RedAdj = nAdj; }

	double TH14K1::GetCP() { return CP; }

	double TH14K1::GetRP() { return 5.0 + RedAdj; }

	double TH14K1::GetPos(const int i) { return pos_table[i]; }
	...
	double TSt113::GetCP() { return CP; }

	double TSt113::GetRP() { return 5.0 + RedAdj; }

	double TSt113::GetEP() { return EPS; }

	double TSt113::GetPos(const int i) { return pos_table[i]; }
	...
	void Ttest::Init(const double Press) { CP = Press; }
	...
	double TFD1::GetCP() { return BP; }

	void TFD1::SetSpeed(const double nSpeed) { Speed = nSpeed; }
	...
	double TFVel6::GetCP() { return CP; }

	double TFVel6::GetRP() { return 5.0; }

	double TFVel6::GetEP() { return EPS; }

	double TFVel6::GetPos(const int i) { return pos_table[i]; }
	...
	double TFVE408::GetCP() { return CP; }

	double TFVE408::GetEP() { return EPS; }

	double TFVE408::GetRP() { return 5.0; }

	double TFVE408::GetPos(const int i) { return pos_table[i]; }
