...
namespace godot {
    ...
    void TrainPart::_process_mover(TMoverParameters *mover, double delta) {
        if (_dirty) {
        ...
        if (enabled) {
            ...
            if (train_controller_node) {
                _do_fetch_state_from_mover(
                        mover, train_controller_node->_get_state_internal()); // FIXME: call from TrainController
            }
        }
