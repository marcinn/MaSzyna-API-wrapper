...
// zawor napelniajacy z PH do PL, PL do LIM
{
    if (LIM > PL) {
        ...
        // bezwymiarowy stosunek cisnien. NOTE: sg is capped at 1 to prevent calculations from going awry. TODO, TBD: log these as errors?
 ...
 */
TBrake::TBrake(const double i_mbp, const double i_bcr, const double i_bcd, const double i_brc, const int i_bcn, const int i_BD,
               int i_mat, const int i_ba, const int i_nbpa) {
    // inherited:: Create;
    MaxBP = i_mbp;
    BCN = i_bcn;
    BCM = 1;
...
double TBrake::GetFC(double const Vel, double const N) const {
    ...
    godot::UtilityFunctions::push_warning("[Maszyna::Brake] : TBrake::GetFC is a null shared pointer");
    return {};
}
...
void TEStEP1::EPCalc(const double dt) {
    ...
    const double dv = PF(S > 0 ? BrakeRes->P() : 0, LBP, abs(S) * (0.00053 + 0.00060 * static_cast<int>(S < 0))) * dt; //przepływ
    LBP = LBP - dv;
}
...
double TLSt::GetPF(double const PP, double const dt, double const Vel) {
    ...
    const bool is_releasing = BrakeStatus & b_rls
    ...
    // cisnienie PP
    RapidTemp =
            RapidTemp + (RM * static_cast<int>(Vel > 55 && BrakeDelayFlag == bdelay_R) - RapidTemp) * dt / 2;
    temp = 1 - RapidTemp;
    if (EDFlag > 0.2)
        temp = 10000;
    double tempasb = 0;
...
double TEStED::GetPF(double const PP, double const dt, double const Vel) {
    ...
    if ((BrakeStatus & b_on) == b_on && BCP < MaxBP)
        dv =
                PF(BVP, BCP, Nozzles[2] * (nastG + 2 * static_cast<int>(BCP < 0.8)) + Nozzles[0] * (1 - nastG)) * dt;
    else
        dv = 0;
    ImplsRes->Flow(-dv);
    BrakeRes->Flow(dv);
    ...
    RapidTemp =
            RapidTemp + (RM * static_cast<int>(Vel > RV && BrakeDelayFlag == bdelay_R) - RapidTemp) * dt / 2;
...
double TMHZ_EN57::GetPF(double i_bcp, const double PP, const double HP, const double dt, double ep) {
    ...
    if ((EQ(i_bcp, -1) && AutoOvrld)
        || (i_bcp < 0.5 && UniversalFlag & ub_Overload)) {
        if (TP < 5)
            TP = TP + dt; // 5/10
...
double TM394::GetPF(const double i_bcp, const double PP, const double HP, const double dt, double ep) {
    ...
    if (BCP != 2)
        if (CP < LimPP)
            ...
            CP = CP +
                 4 * (1 + static_cast<int>(BCP != 3) + static_cast<int>(BCP > 4)) * Min0R(abs(LimPP - CP), 0.05) *
                 PR(CP, LimPP) * dt; // zbiornik sterujący
...
double TH14K1::GetPF(const double i_bcp, const double PP, const double HP, double dt, double ep) {
    ...
    if (LimPP < 0.0) { LimPP = 0.5 * PP; } else if (LimPP > 0.0) { LimPP = PP; } else { LimPP = CP; }
...
double TFD1::GetPF(const double i_bcp, double PP, const double HP, double dt, double ep) {
    ...
    double DP = 10.0 * std::min(std::abs(temp - BP), 0.1) * PF(temp, BP, 0.0006 * (temp > BP ? 3.0 : 2.0)) * dt * Speed;
    BP = BP - DP;
    return -DP;
}
...
double TFVE408::GetPF(const double i_bcp, const double PP, const double HP, const double dt, double ep) {
    ...
    if (is_EQ(i_bcp, 1)) EPS = 1.15;
    else if (is_EQ(i_bcp, 2)) EPS = 1.40;
    else if (is_EQ(i_bcp, 3)) EPS = 2.64;
    else if (is_EQ(i_bcp, 4)) EPS = 3.84;
    else if (is_EQ(i_bcp, 5)) EPS = 3.99;
    else
        EPS = 0;
